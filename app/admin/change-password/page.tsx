...
export default function AdminChangePasswordPage() {
  ...
  useEffect(() => {
    ...
    const validateLink = async () => {
      if (!email || !token) {
        if (mounted) {
          setStatus({ type: "error", text: "Invalid password change link." });
          setIsChecking(false);
        }
        return;
      }
      ...
      try {
        await request("/api/admin/super-admin/password-change/validate", {
          method: "POST",
        ...
        if (mounted) {
          setStatus({
            type: "error",
            text: error instanceof Error ? error.message : "Password change link validation failed.",
          });
        }
      } finally {
        if (mounted) {
  ...
  const handleSubmit = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    setIsLoading(true);
    try {
      const data = await request("/api/admin/super-admin/password-change/confirm", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          email,
      ...
      setStatus({
        type: "success",
        text: (data as { message?: string })?.message || "Password changed successfully. Please login again.",
      });
          ...
          <aside className="relative hidden overflow-hidden border-r border-[rgba(15,76,129,0.08)] p-6 lg:flex lg:flex-col lg:justify-between xl:p-8">
            ...
            <div className="relative z-10">
              ...
              <div className="mt-5 inline-flex rounded-full border border-[rgba(15,76,129,0.12)] bg-[rgba(15,76,129,0.06)] px-4 py-2 text-xs font-semibold uppercase tracking-[0.18em] text-[color:var(--brand-primary)]">
                <MailCheck className="mr-2 size-3.5" />
                Admin Security
              </div>
            </div>
            ...
            <div className="relative z-10 rounded-[2rem] border border-[rgba(15,76,129,0.1)] bg-[linear-gradient(180deg,#ffffff,#eff6ff)] p-8 shadow-[0_22px_50px_rgba(22,50,79,0.1)]">
              ...
              <p className="mt-4 text-sm leading-7 text-[color:var(--text-muted)]">
                This link verifies your admin email first. Then we ask for your current password
                and the new password before updating access.
              </p>
            </div>

            <p className="relative z-10 max-w-sm text-sm leading-7 text-[color:var(--text-muted)]">
              Password is never shown in the dashboard or stored in frontend code. This page only
              works with a valid mail link.
            </p>
          </aside>
          ...
          <main className="flex items-center justify-center p-4 sm:p-6 md:p-8 lg:p-10 xl:p-12">
            <div className="w-full max-w-md sm:max-w-lg lg:max-w-md">
              <div className="mb-7 text-center">
                ...
                <h1 className="text-3xl font-bold text-[color:var(--text-dark)]">Admin Password Change</h1>
