...
const buildForm = (college: Record<string, unknown> | null, user: SafeAuthUser | null) => {
  ...
  const hostelFees =
    (hostel.feesStructure as Record<string, unknown>) ||
    (hostel.hostelFees as Record<string, unknown>) ||
    {};
  const wifi = (hostel.wifi as Record<string, unknown>) || {};
  const hostelFacilities = Array.isArray(hostel.facilities)
    ? hostel.facilities
    : Array.isArray(hostel.facilityOptions)
      ? hostel.facilityOptions
      : [];
  const hostelAvailability =
    String(hostel.availability || "").trim() ||
    (Boolean(college?.hasHostel) ? "available" : "not_available");
  return {
...
export function CollegeDashboardAddCollegeForm({ token, currentUser, college, courses = [], requestActionType, approvalMessage, onSaved }: Props) {
  ...
  const saveCollege = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      const hostelFacilities = hasHostel ? toList(next.hostelFacilityOptions) : [];
      const payload = { ...next, image: next.coverImage, brochureUrl: next.brochurePdfUrl, mapUrl: next.locationLink, courseTags: toList(next.courseTags), facilities: toList(next.facilities), quotas: toList(next.quotas), placementRate: next.placementRate, highestPackage: next.highestPackage, averagePackage: next.averagePackage, companiesVisited: next.companyCount, hasHostel, feesStructure: { min: next.feeMin, max: next.feeMax }, placements: { averagePackage: next.averagePackage, highestPackage: next.highestPackage, placementRate: next.placementRate, companiesVisited: next.companyCount }, hostelDetails: { availability: hasHostel ? "available" : "not_available", hostelType: hasHostel ? next.hostelType : "", feesStructure: { min: hasHostel ? next.hostelFeeMin : "", max: hasHostel ? next.hostelFeeMax : "" }, hostelFees: { minAmount: hasHostel ? next.hostelFeeMin : "", maxAmount: hasHostel ? next.hostelFeeMax : "" }, cctvAvailable: hasHostel ? next.cctvAvailable : "", facilities: hostelFacilities, facilityOptions: hostelFacilities, waterAvailability: hasHostel ? next.waterAvailability : "", powerBackup: hasHostel ? next.powerBackup : "", wifi: { available: hasHostel ? next.wifiAvailable : "", speed: hasHostel ? next.wifiSpeed : "", pricing: hasHostel ? next.wifiPricing : "" }, foodAvailability: hasHostel ? next.foodAvailability : "", foodTimings: hasHostel ? next.foodTimings : "", laundryService: hasHostel ? next.laundryService : "", roomCleaningFrequency: hasHostel ? next.roomCleaningFrequency : "", rules: hasHostel ? next.hostelRules : "" }, approvedFromCollegeRequest: requestActionType === "update" ? "update" : "create" };
  ...
  const courseStepContent = (
    <div className="space-y-4">
      <div className="flex flex-col gap-3 rounded-[1rem] border border-slate-200 bg-slate-50 p-4 sm:flex-row sm:items-center sm:justify-between">
        <div>
      ...
      {showCourseEditor ? (
        <div className="space-y-4 rounded-[1rem] border border-slate-200 bg-slate-50 p-4">
          <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
            ...
            <button type="button" onClick={resetCourseEditor} className={`${softButton} w-full sm:w-auto`}>Cancel</button>
          </div>
          ...
          <div className={sectionClass}>
            ...
            <div className="md:col-span-2 xl:col-span-3">
              ...
              <div className="grid gap-2 md:grid-cols-[180px_minmax(0,1fr)_auto]">
                ...
                <button type="button" onClick={upsertCourseCutoff} className={`${primaryButton} w-full md:w-auto`}>Add Cutoff</button>
              </div>
            ...
            <div className="rounded-[1rem] border border-slate-200 bg-white p-3">
              <div className="mb-3 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                ...
                <div className="flex flex-col gap-2 sm:flex-row sm:items-center">
                  ...
                  <button type="button" onClick={() => setCourseForm((prev) => ({ ...prev, entranceExams: [...prev.entranceExams, emptyExam()] }))} className={`${softButton} w-full sm:w-auto`}><Plus className="size-4" />Add Exam</button>
                </div>
              </div>
          ...
          ) : <div className="rounded-[1rem] border border-dashed border-slate-300 bg-white p-3"><div className="flex flex-col gap-2 sm:flex-row sm:flex-wrap"><button type="button" onClick={() => setCourseForm((prev) => ({ ...prev, entranceExamsEnabled: true, entranceExams: prev.entranceExams.length ? prev.entranceExams : [emptyExam()] }))} className={`${softButton} w-full sm:w-auto`}><Plus className="size-4" />Add Entrance Exam</button><button type="button" onClick={() => setCourseForm((prev) => ({ ...prev, entranceExamsEnabled: false, entranceExams: [emptyExam()] }))} className={`${softButton} w-full sm:w-auto`}>Entrance Exam Not Needed</button></div></div>}

          <div className="flex flex-col gap-2 sm:flex-row">
            <button type="button" onClick={saveCourseDraft} className={`${primaryButton} w-full sm:w-auto`}>{editingCourseId ? "Update Course" : "Save Course"}</button>
            <button type="button" onClick={resetCourseEditor} className={`${softButton} w-full sm:w-auto`}>Cancel</button>
          </div>
        </div>
      ) : null}
          ...
          <div className="mt-3 space-y-3">
            {courseDrafts.map((item) => (
              <div key={item.id} className="flex flex-col gap-3 rounded-[1rem] border border-slate-200 bg-white p-3 sm:flex-row sm:items-center sm:justify-between">
                ...
                <div className="flex flex-col gap-2 sm:flex-row">
                  <button type="button" onClick={() => { setEditingCourseId(item.id); setCourseForm(item); setShowCourseEditor(true); }} className={`${softButton} w-full sm:w-auto`}>Edit</button>
                  <button type="button" onClick={() => { if (item.persisted) { setRemovedCourseIds((prev) => prev.includes(item.id) ? prev : [...prev, item.id]); } setCourseDrafts((prev) => prev.filter((course) => course.id !== item.id)); }} className={`${softButton} w-full sm:w-auto`}>Remove</button>
                </div>
              </div>
            ))}
          </div>
    ...
    <article className="luxe-card reveal-up delay-3 rounded-[1.35rem] p-3.5 sm:p-4">
      ...
      <form onSubmit={saveCollege} className="mt-4 rounded-[1.35rem] border border-white/80 bg-[linear-gradient(180deg,#ffffff_0%,#f8fbff_100%)] p-3 text-sm shadow-[0_24px_46px_rgba(148,163,184,0.14)] sm:p-4">
        <div className="rounded-[1.15rem] border border-slate-200 bg-slate-50 p-3">
          ...
          <div className="-mx-1 mt-3 flex gap-2 overflow-x-auto px-1 pb-1 sm:mx-0 sm:grid sm:grid-cols-5 sm:overflow-visible sm:px-0 sm:pb-0 xl:grid-cols-10">{steps.map((item, index) => <button key={item} type="button" onClick={() => goToStep(index)} className={`min-w-[8.5rem] shrink-0 rounded-[0.95rem] border px-2.5 py-2 text-left text-[11px] font-semibold transition sm:min-w-0 ${index === step ? "border-slate-900 bg-slate-900 text-white" : "border-slate-200 bg-white text-slate-500"}`}>{item}</button>)}</div>
        </div>
        <div className="mt-4 space-y-4">
          ...
          {step === 8 ? <div className="space-y-4"><div className="flex flex-wrap gap-2"><button type="button" onClick={() => setField("hostelAvailability", "available")} className={`rounded-full px-4 py-2 text-sm font-semibold transition ${hasHostel ? "bg-slate-900 text-white" : "border border-slate-200 bg-white text-slate-700"}`}>Hostel Available</button><button type="button" onClick={() => setField("hostelAvailability", "not_available")} className={`rounded-full px-4 py-2 text-sm font-semibold transition ${!hasHostel ? "bg-slate-900 text-white" : "border border-slate-200 bg-white text-slate-700"}`}>Hostel Not Available</button></div>{hasHostel ? <div className={sectionClass}><label><span className={labelClass}>Hostel Type *</span><select className={inputClass} value={form.hostelType} onChange={(e) => setField("hostelType", e.target.value)}><option value="">Hostel type</option>{hostelTypeOptions.map((item) => <option key={item.value} value={item.value}>{item.label}</option>)}</select></label><label className="md:col-span-2 xl:col-span-2"><span className={labelClass}>Hostel Fees Structure *</span><div className="grid gap-2 sm:grid-cols-2"><input className={inputClass} value={form.hostelFeeMin} onChange={(e) => setField("hostelFeeMin", e.target.value)} placeholder="Min fee" /><input className={inputClass} value={form.hostelFeeMax} onChange={(e) => setField("hostelFeeMax", e.target.value)} placeholder="Max fee" /></div></label><label><span className={labelClass}>CCTV Availability *</span><select className={inputClass} value={form.cctvAvailable} onChange={(e) => setField("cctvAvailable", e.target.value)}><option value="">CCTV availability</option>{yesNoOptions.map((item) => <option key={item.value} value={item.value}>{item.label}</option>)}</select></label><label className="md:col-span-2 xl:col-span-3"><span className={labelClass}>Facilities</span><input className={inputClass} value={form.hostelFacilityOptions} onChange={(e) => setField("hostelFacilityOptions", e.target.value)} /></label></div> : <div className="rounded-[1rem] border border-dashed border-slate-200 bg-slate-50 p-4 text-sm text-slate-500">Select Hostel Available to enter hostel details.</div>}</div> : null}
        ...
        <div className="mt-5 flex flex-col gap-2 border-t border-slate-100 pt-4 sm:flex-row sm:items-center sm:justify-between"><div className="flex items-center gap-2 text-xs text-slate-500"><BadgeCheck className="size-4" /><span>Step {step + 1} of {steps.length}</span></div><div className="flex flex-col gap-2 sm:flex-row sm:flex-wrap"><button type="button" onClick={() => setStep((prev) => Math.max(prev - 1, 0))} className={`${softButton} w-full sm:w-auto`} disabled={step === 0}><ArrowLeft className="size-4" />Previous</button>{step < steps.length - 1 ? <button type="button" onClick={() => goToStep(step + 1)} className={`${softButton} w-full sm:w-auto`}>Next<ArrowRight className="size-4" /></button> : null}<button type="submit" className={`${primaryButton} w-full sm:w-auto`} disabled={saving}>{saving ? "Saving..." : "Save College"}</button></div></div>
      </form>
    </article>
  );
}
