...
export function CollegeDashboardAddCollegeForm({ token, currentUser, college, requestActionType, approvalMessage, onSaved }: Props) {
  ...
  useEffect(() => { setForm(buildForm(college, currentUser)); setCourseForm(emptyCourseDraft(text(college, "university"))); }, [college, currentUser]);
  ...
  const saveCollege = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      const next = { ...form, logo: uploaded.logo || form.logo, coverImage: uploaded.coverImage || form.coverImage, images: uploaded.images.length ? uploaded.images : form.images, brochurePdfUrl: uploaded.brochurePdfUrl || form.brochurePdfUrl, ranking: formatRankingRangeForSave(form.ranking) };
      ...
      if (!next.logo.trim() || !next.coverImage.trim() || next.images.length === 0) return setStatus({ type: "error", text: "Media fields are required" });
    ...
    <article className="luxe-card reveal-up delay-3 rounded-[1.35rem] p-3.5 sm:p-4">
      ...
      <form onSubmit={saveCollege} className="mt-4 rounded-[1.35rem] border border-white/80 bg-[linear-gradient(180deg,#ffffff_0%,#f8fbff_100%)] p-3 text-sm shadow-[0_24px_46px_rgba(148,163,184,0.14)] sm:p-4">
        ...
        <div className="mt-4 space-y-4">
          ...
          {step === 2 ? <div className={sectionClass}><label><span className={labelClass}>Official Email *</span><input className={inputClass} value={form.contactEmail} onChange={(e) => setField("contactEmail", e.target.value)} /></label><label><span className={labelClass}>Phone Number *</span><input className={inputClass} value={form.contactPhone} onChange={(e) => setField("contactPhone", e.target.value)} /></label><label><span className={labelClass}>Alternate Phone</span><input className={inputClass} value={form.alternatePhone} onChange={(e) => setField("alternatePhone", e.target.value)} /></label><label><span className={labelClass}>Website URL</span><input className={inputClass} value={form.website} onChange={(e) => setField("website", e.target.value)} /></label></div> : null}
          {step === 3 ? <div className={sectionClass}><label><span className={labelClass}>Logo Image *</span><input className={inputClass} type="file" accept="image/*" onChange={(e) => setLogoFile(e.target.files?.[0] || null)} /></label><label><span className={labelClass}>Cover Image *</span><input className={inputClass} type="file" accept="image/*" onChange={(e) => setCoverImageFile(e.target.files?.[0] || null)} /></label><label><span className={labelClass}>College Images *</span><input className={inputClass} type="file" accept="image/*" multiple onChange={(e) => setImageFiles(Array.from(e.target.files || []))} /></label><label><span className={labelClass}>Brochure PDF</span><input className={inputClass} type="file" accept="application/pdf" onChange={(e) => setBrochureFile(e.target.files?.[0] || null)} /></label><label className="md:col-span-2 xl:col-span-2"><span className={labelClass}>Campus Video</span><input className={inputClass} value={form.campusVideoUrl} onChange={(e) => setField("campusVideoUrl", e.target.value)} /></label></div> : null}
          ...
          {step === 6 ? <div className={sectionClass}><label><span className={labelClass}>Quotas</span><input className={inputClass} value={form.quotas} onChange={(e) => setField("quotas", e.target.value)} /></label><label><span className={labelClass}>Minimum Fee *</span><input className={inputClass} value={form.feeMin} onChange={(e) => setField("feeMin", e.target.value)} /></label><label><span className={labelClass}>Maximum Fee *</span><input className={inputClass} value={form.feeMax} onChange={(e) => setField("feeMax", e.target.value)} /></label><label className="md:col-span-2 xl:col-span-2"><span className={labelClass}>Admission Process *</span><textarea className={inputClass} rows={3} value={form.admissionProcess} onChange={(e) => setField("admissionProcess", e.target.value)} /></label><label><span className={labelClass}>Application Mode *</span><select className={inputClass} value={form.applicationMode} onChange={(e) => setField("applicationMode", e.target.value)}><option value="">Select application mode</option>{applicationModeOptions.map((item) => <option key={item} value={item}>{item}</option>)}</select></label><label className="md:col-span-2 xl:col-span-3"><span className={labelClass}>Scholarships</span><textarea className={inputClass} rows={2} value={form.scholarships} onChange={(e) => setField("scholarships", e.target.value)} /></label></div> : null}
          {step === 7 ? <div className={sectionClass}><label><span className={labelClass}>Average Package</span><input className={inputClass} value={form.averagePackage} onChange={(e) => setField("averagePackage", e.target.value)} /></label><label><span className={labelClass}>Highest Package</span><input className={inputClass} value={form.highestPackage} onChange={(e) => setField("highestPackage", e.target.value)} /></label><label><span className={labelClass}>Placement Percentage</span><input className={inputClass} value={form.placementRate} onChange={(e) => setField("placementRate", e.target.value)} /></label><label><span className={labelClass}>Companies Visited</span><input className={inputClass} value={form.companyCount} onChange={(e) => setField("companyCount", e.target.value)} /></label></div> : null}
